...
def generate_scalping_signal(symbol):
    ...
    try:
        ...
        df["vwap"] = (df["volume"] * (df["high"] + df["low"] + df["close"]) / 3).cumsum() / df["volume"].replace(0, 1).cumsum()
        df["rsi"] = ta.momentum.RSIIndicator(close=df["close"], window=14).rsi()
        ...
        elif latest["rsi"] <= 45 and latest["rsi"] >= 30:
            score -= 1
        ...
        return {
            ...
            "avg_volume": round(float(latest["avg_volume"]), 2) if pd.notna(latest["avg_volume"]) else 0,
            "reasons": reasons,
        }
